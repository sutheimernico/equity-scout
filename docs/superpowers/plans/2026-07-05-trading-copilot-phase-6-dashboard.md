...
- [ ] **Step 1: Build the component** + any `EquityChart` multi-series extension (keep the existing single-series callers working — add an optional `series?: {label,points,color,dashed}[]` prop that, when present, supersedes the primary/benchmark props; document the back-compat). Colors from tokens only. SVG `aria-label` describing the comparison. `format.eur`/`pct`/`maxDrawdown`.
- [ ] **Step 2: Gate** typecheck + build (add Arena nav entry + mount; make Arena the DEFAULT view). Green.
- [ ] **Step 3: Commit** `feat(ui): add arena surface — Du vs Autopilot vs Markt equity race`.
