...
- [ ] **R1 (P0) session lane: force-flat stale positions** — `src/equity_scout/st_session.py`:
...
- [ ] **R2 (P1) central SQLite conventions** — new `src/equity_scout/db.py`: `connect(path)` →
...
- [ ] **R3 (P1) atomic autotrader persist** — `autotrader_storage.py`: new
...
- [ ] **R4 (P1) atomic shortterm persist** — `shortterm_storage.py:91,108,132,168`: same
...
- [ ] **R5 (P1) depot position exits via sleeve books** — the honest fix for "depot has no
...
- [ ] **R6 (P1) digest delivery: persist + resend** — `scripts/run_digest.py:296-307`: on
...
- [ ] **R7 (P1) digest freshness guards** — `digest.py` Auto-Depot block + Arena block: when
...
- [ ] **R8 (P1) intraday bars tz assertion** — `src/equity_scout/intraday_bars.py:34-56`:
...
- [ ] **R9 (P2) allocator calendar gaps** — `autotrader_allocator.py:44-58`
...
- [ ] **R10 (P2) market hours from NYSE calendar** — `market_hours.py:15-27`: compute the
...
- [ ] **R11 (P2) small robustness sweep** — (a) `api.py:462-498` `/api/entry/{ticker}`: wrap
...
- [ ] **W1 chain heartbeats + dead-man watchdog** — every guarded chain entry
...
- [ ] **W2 auto-depot event push** — after a successful autotrader advance with ≥1 trade or
...
- [ ] **I1 total-wealth API** — `api.py` `/api/overview`: one payload with per-book equity +
...
- [ ] **I2 promotion gate module** — new `src/equity_scout/promotion.py`:
  ...
  all pass → eligible.
- [ ] **I3 promotion wiring (status-first)** — `scripts/run_autotrader.py`: at each advance,
...
- [ ] **I4 integration surfaces** — digest: Arena block gains one line per lane
...
- [ ] **M1 LAN bind + token auth** — `scripts/run_api.py`: `--host` flag (default
...
- [ ] **M2 PWA shell** — `frontend/`: `public/manifest.webmanifest` (name "Equity Scout",
...
- [ ] **M3 dashboard server as a service** — `scripts/systemd/equity-scout-dash.service`
...
- [ ] **M4 phone onboarding** — digest gains a one-time-per-week footer line with the
...
- [ ] **P1 proof metrics module** — new `src/equity_scout/proof.py`:
...
- [ ] **P2 proof surfaces** — `api.py` `/api/proof` (report per book: autodepot, lanes,
...
- [ ] **P3 monthly telegram report** — on the 1st of each month (state-gated
...
- [ ] **P4 docs closure** — README: "Kann das funktionieren?" section (what the system
