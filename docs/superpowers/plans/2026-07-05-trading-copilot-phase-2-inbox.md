...
**Review fix round 2** (bundled hardening commit with Tasks 1/6/7 items): (a) **loop survives outages** — `main()`'s while-loop wraps the round in `try/except (TelegramError, OSError)` → German stderr warning + `time.sleep(5)` backoff + continue (`time.sleep` in `main()` only, matching `run_research.py` precedent); failed rounds still count toward `--rounds` so supervised runs terminate. DECISION: no offset persistence in v1 — Telegram redelivers unconfirmed updates and `decide_pitch` is idempotent (first decision wins), so replay after a restart is safe; documented in the module docstring. (b) **acks/edits best-effort** — `answer()`/`edit()` each wrapped via `_try_telegram` (warn + continue): a failed ack or edit can never lose a decision or abort the round. (c) **stuck-edit self-heal** — the already-decided branch re-attempts the outcome edit with the STORED decision's label and `decided_at` when the pitch exists, is decided, and has a `telegram_message_id`; idempotent because an unchanged message yields Telegram's "message is not modified" error, swallowed by (b). (d) receiver reuses `telegram_client.DECISION_LABELS` instead of duplicating the dict. TDD: `test_process_round_survives_answer_and_edit_failures`, `test_duplicate_press_on_decided_pitch_reattempts_edit`, `test_main_survives_round_errors_with_backoff` (get_updates raising, sleep monkeypatched, asserts two 5s backoffs + exit 0).
...
**Review fix round 2** (bundled hardening commit): the digest was a lifetime archive, not a daily digest — every decided pitch reappeared forever (capped only by `load_pitches`' incidental default `limit=100`). `build_digest` gained `decided_since: str | None = None`; the decided section is filtered to `decided_at >= decided_since` when provided (lexicographic compare, valid per the UTC-"+00:00" writer invariant). `run_digest.py` passes `decided_since = now - 24h` and loads with `limit=1000` so open pitches are never silently dropped. Wording: `"Offene Pitches: {n}"` count style (dodges singular/plural agreement of the old `"Offene Pitches (1):"`), `"✅ gekauft-Entscheidung"` → `"✅ Kaufentscheidung"`. TDD: `test_build_digest_decided_since_window_pins_line_and_drops_old` pins the exact rendered decided line (`"  ✅ Kaufentscheidung — ABC · am 2026-07-05"`) and asserts an out-of-window pitch is absent.
...
**Review fix round 2** (bundled hardening commit): (a) **unified error shape** — the 409 was the file's first `HTTPException` (`{"detail": ...}`) while the 422 used the file-wide `{"error": ...}` idiom; now both return `JSONResponse({"error": ...})`, `HTTPException` import removed. (b) **overflow guard** — `POST /api/inbox/99999999999999999999/decision` previously hit sqlite's `OverflowError` → 500; `inbox_storage.decide_pitch` now returns `False` unless `0 <= pitch_id < 2**63` (outside SQLite's signed 64-bit INTEGER range no row can exist), guarding the route AND the receiver; tested at the API level (huge id → 409). (c) **decision POST returns the updated pitch row** (fetched after decide, same limit-1000 scan idiom as the receiver) alongside `ok` + `disclaimer`, so the dashboard can update in place — test asserts `status == "buy"` and a non-null `decided_at`.
