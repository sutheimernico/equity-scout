...
def record_heartbeat(db_path: str, chain: str, *, now: str) -> None:
    """Liveness marker per chain (v12 W1): written on every successful chain run; the
    watchdog alarms when one goes stale. ISO timestamps, lexicographically comparable."""
    set_state(db_path, key=f"heartbeat_{chain}", value=now)
